package com.lmcat.ncis;

import java.util.ArrayList;
import java.util.Scanner;

/*
https://github.com/jtbrockway/Math3522/blob/master/NCIS.java
Test Cases:
#1)
Input:
	(1, 2), (2, 3), (3, 5)
...
public class NCIS {
    ...
    public static void ncisAlgo(ArrayList<double[]> xy, ArrayList<Double> bs, ArrayList<Double> cs, ArrayList<Double> ds){
        ...
        for(int j = 0; j < xy.size() - 1; j++){
            ...
            System.out.println("    " + si + aj + " + " + bj + "(x - " + xj + ") + " + cj + "(x - " + xj + ")^2 + " + dj + "(x - " + xj +")^3" + "    (" + Double.toString(xy.get(j)[0])  + " <= x <= " + Double.toString(xy.get(j+1)[0]) + ")");
        }
    }
